...
function create_release() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    # Write combined notes to a temp file (stdin piping via --notes - is unreliable).
    NOTES_FILE=$(mktemp)
    printf '%s\n\n---\n\n%s\n' "$release_notes" "$autogenerated_release_notes" > "$NOTES_FILE"
    gh release edit "$version" --title "$release_title" --notes-file "$NOTES_FILE"
    rm -f "$NOTES_FILE"
